...
points = []  # 마우스가 지나간 모든 좌표를 저장할 리스트
canvas = None # 그림이 그려질 투명 레이어 (웹캠 크기에 맞춰 생성 예정)
...
async def main():
    ...
    async with BleakClient(device) as client:
        ...
        def get_data():
            ...
            # 고도 제어 (Throttle): 배터리 잔량에 따라 조정 필요
            ...
            # D항: (현재오차 - 이전오차) = 변화량(속도)
            # ---------------------------------------------------------
            # 목표 지점이 변경된 직후에는 D항(미분)을 적용하지 않아 급격한 반응 방지
        ...
        try:
            print("[키] q: 종료, r: 녹화 시작/종료, d: 그림 지우기")
            ...
            while True:
                ...
                combined_img = cv2.add(frame, canvas)

                cv2.imshow('Paint', combined_img)
                ...
                if key == ord('d'): # 지우기
                    canvas = np.zeros((h, w, 3), np.uint8) # 캔버스 초기화
                    points = []
